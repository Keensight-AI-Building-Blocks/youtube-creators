...
video_ideas_prompt = PromptTemplate(
    ...
    partial_variables={"format_instructions": video_ideas_parser.get_format_instructions()},
)

analyze_comments_prompt = PromptTemplate(
    template="Analyze the comments for a video.\n{format_instructions}\n{input}\n",
    input_variables=["input"],
    partial_variables={"format_instructions": comment_analysis_parser.get_format_instructions()},
)